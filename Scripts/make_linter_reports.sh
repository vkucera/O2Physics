...
cmd_clang_tidy="clang-tidy-20 {} --config-file=\"$dir_alice/O2Physics/.clang-tidy\""
cmd_cppcheck="$HOME/projects/cppcheck/cppcheck/build/bin/cppcheck {} --language=c++ --std=c++20 --enable=style --check-level=exhaustive --suppressions-list=\"$dir_alice/O2Physics/cppcheck_suppressions\" --inline-suppr --force"
